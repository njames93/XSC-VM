...
void xscScript::BreakPoint(bool pause_execution)
{
	std::string Message = _Name + ": Breakpoint hit at position " + HexToString(_codeTableIndex+1/*Dont need to display throw opcode*/) + ", Next opcode = " + HexToString(_codeTable[_codeTableIndex + 1]) + "\n";
...
bool xscScript::OnTick()
{
	...
	for (;; _codeTableIndex++)
	{
		...
		{
			Log(_Name + ": Exception caught, current opcode pointer = " + HexToString(_codeTableIndex) + ", opcode = " + HexToString(_codeTable[_codeTableIndex]));
			Terminate();
			return true;
		}
	}
